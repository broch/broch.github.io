...
The telemetry ecosytem can be a bit daunting for a newcomer. For example, if you have been using log files all your life but have just discovered Rust's [tracing](https://tokio.rs/#tk-lib-tracing) system, it's not immediately clear how things fit together. There are a lot of crates and APIs to get your head round. This article isn't intended to be a complete tutorial but will explain how to set up a working configuration and hopefully provide some insight into how things fit together. For an overview of telemetry, the [OpenTelemetry Observability Primer](https://opentelemetry.io/docs/concepts/observability-primer) is a quick introduction. Note that tracing can theoretically take place across multiple processes, but we'll only be dealing with exporting data from a single web application here. Source code for a working application can be found [on github](https://github.com/tekul/rust-tracing-otlp).

## The Rust tracing and telemetry ecosystem

Rust has its own [tracing framework](https://crates.io/crates/tracing). Getting started and with some logging to `stdout` is easy. You can just add:
...
Some of these crates are maintained by the tokio tracing team and some are part of OpenTelemetry organization. It's important that the versions are compatible or you can get errors as everything is still quite unstable for now. Some of the code also require specific crate features to be enabled. The `Cargo.toml` file from the [example code](https://github.com/tekul/rust-tracing-otlp) can be used as a starting point.

## Basic Tracing Setup

Rust tracing allows us to instrument our code to generate events and we then need to define at least one [`Subscriber`](https://docs.rs/tracing-core/0.1.30/tracing_core/subscriber/trait.Subscriber.html) to capture and log these events somewhere. The code `tracing_subscriber::fmt().init();` creates a simple subscriber which, by default, writes timestamped output to the console similar to a traditional log.
...
Now pull the docker image for `jaeger/all-in-one:latest` and then run the server:
...
In the previous example, the exporter uses default endpoint which happens to be where our Jaeger server is listening (`http://127.0.0.1:4317`). The OpenTelemetry SDK in theory allows us to override the endpoint and other settings by using [predefined environment variables](https://opentelemetry.io/docs/concepts/sdk-configuration/otlp-exporter-configuration/).
...
For maximum flexibility we might also want to be able to use the `http/proto` protocol rather than being restricted to gRPC. So the final version of the example code reads the value of the [`OTEL_EXPORTER_OTLP_PROTOCOL`](https://opentelemetry.io/docs/reference/specification/protocol/exporter/#specify-protocol) environment variable to decide what to use (it defaults to "grpc" if it's not set).
...
There's a lot more to tracing and telemetry than what we've covered here, but we've managed to produce a setup that should work with different OpenTelemetry compatible providers, without the need to recompile. We can easily configure it using standard environment variables (which will hopefully be supported directly by the opentelemetry libraries some day). This gives us local event logging along with exporting of tracing spans to an OpenTelemetry traces endpoint.
